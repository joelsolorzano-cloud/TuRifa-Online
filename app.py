...
app = Flask(__name__)
# Configura el soporte de CORS (necesario para la comunicación entre frontend y backend)
CORS(app) 
...
with app.app_context():
    ...
    # Si la tabla está vacía, inserta 10 boletos de prueba
    if Boleto.query.count() == 0:
        for i in range(1, 11):
            db.session.add(Boleto(asiento=f'A{i}', estado='DISPONIBLE'))
        db.session.commit()
    print("Base de datos inicializada con 10 boletos de ejemplo (A1 a A10).")
...
if __name__ == '__main__':
    # Ejecuta el servidor en modo depuración. Accesible en http://127.0.0.1:5000/
    app.run(debug=True)